...
final class LoggerPrinter implements IPrinter {
   ...
   */
  private final LogSettings settings = new LogSettings();
   ...
   */
  @Override public LogSettings init(String tag) {
    if (tag == null) {
  ...
  @Override public LogSettings getSettings() {
    return settings;
  }

  @Override public IPrinter t(String tag, int methodCount) {
    if (tag != null) {
      localTag.set(tag);
    }
  ...
  private String formatTag(String tag) {
    if (!LogHelper.isEmpty(tag) && !LogHelper.equals(this.tag, tag)) {
      return this.tag + "-" + tag;
    }
    return this.tag;
  }
