...
class FragmentationDelegate {
    ...
    FragmentationDelegate(ActivitySupport activity) {
     ...
     * @param to          目标Fragment
     */
    void loadRootTransaction(FragmentManager fragmentManager, int containerId, FragmentSupport to) {
        FragmentSupport fragment = findFragmentByRoot(fragmentManager, to.getClass().getName());
        if (fragment != null && containerId == fragment.getContainerId()) {
            return;
        }
        bindContainerId(containerId, to);
        dispatchStartTransaction(fragmentManager, null, to, 0, FragmentSupport.STANDARD, TYPE_ADD);
    }
     ...
     * @param to          目标Fragment
     */
    void replaceLoadRootTransaction(FragmentManager fragmentManager, int containerId, FragmentSupport to, boolean addToBack) {
     ...
     * 加载多个根Fragment
     */
    void loadMultipleRootTransaction(FragmentManager fragmentManager, int containerId, int showPosition, FragmentSupport... tos) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
        FragmentTransaction ft = fragmentManager.beginTransaction();
        for (int i = 0; i < tos.length; i++) {
            FragmentSupport to = tos[i];
     ...
     * @param type        类型
     */
    void dispatchStartTransaction(FragmentManager fragmentManager, FragmentSupport from, FragmentSupport to, int requestCode, int launchMode, int type) {
    ...
    private void bindContainerId(int containerId, FragmentSupport to) {
     ...
     */
    void replaceTransaction(FragmentSupport from, FragmentSupport to, boolean addToBack) {
     ...
     * replace事务, 主要用于子Fragment之间的replace
     */
    void replaceTransaction(FragmentManager fragmentManager, int containerId, FragmentSupport to, boolean addToBack) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
     ...
     */
    void showHideFragment(FragmentManager fragmentManager, FragmentSupport showFragment, FragmentSupport hideFragment) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return;
    ...
    void start(FragmentManager fragmentManager, FragmentSupport from, FragmentSupport to, String toFragmentTag, ArrayList<TransactionRecord.SharedElement> sharedElementList) {
        FragmentTransaction ft = fragmentManager.beginTransaction();
    ...
    void startWithPop(final FragmentManager fragmentManager, FragmentSupport from, FragmentSupport to, String toFragmentTag) {
        ...
        FragmentSupport preFragment = getPreFragment(from);
     ...
     * 获得栈顶SupportFragment
     */
    FragmentSupport getTopFragment(FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--) {
            ...
            if (fragment instanceof FragmentSupport) {
                return (FragmentSupport) fragment;
            }
        }
        return null;
    }
     ...
     */
    FragmentSupport getPreFragment(Fragment fragment) {
        ...
        for (int i = index - 1; i >= 0; i--) {
            ...
            if (preFragment instanceof FragmentSupport) {
                return (FragmentSupport) preFragment;
            }
        }
        return null;
    }
    ...
    <T extends FragmentSupport> T findStackFragment(Class<T> fragmentClass, String toFragmentTag, FragmentManager fragmentManager) {
        fragmentManager = checkFragmentManager(fragmentManager, null);
        if (fragmentManager == null) return null;
        ...
        if (toFragmentTag == null) {
            ...
            for (int i = sizeChildFrgList - 1; i >= 0; i--) {
                ...
                if (brotherFragment instanceof FragmentSupport && brotherFragment.getClass().getName().equals(fragmentClass.getName())) {
     ...
     */
    private FragmentSupport findFragmentByRoot(FragmentManager fragmentManager, String tag) {
        List<Fragment> fragmentList = fragmentManager.getFragments();
        if (fragmentList != null) {
            for (Fragment fragment : fragmentList) {
                if (fragment instanceof FragmentSupport && tag.equals(fragment.getTag())) {
                    return (FragmentSupport) fragment;
                }
            }
        }
     ...
     */
    FragmentSupport getActiveFragment(FragmentSupport parentFragment, FragmentManager fragmentManager) {
        List<Fragment> fragmentList = fragmentManager.getFragments();
        if (fragmentList == null) {
        ...
        for (int i = fragmentList.size() - 1; i >= 0; i--) {
            Fragment fragment = fragmentList.get(i);
            if (fragment instanceof FragmentSupport) {
                FragmentSupport supportFragment = (FragmentSupport) fragment;
     ...
     */
    boolean dispatchBackPressedEvent(FragmentSupport activeFragment) {
        if (activeFragment != null) {
            ...
            if (dispatchBackPressedEvent((FragmentSupport) parentFragment)) {
                return true;
            }
        }
     ...
     * handle LaunchMode
     */
    private boolean handleLaunchMode(FragmentManager fragmentManager, FragmentSupport toFragment, String toFragmentTag, int launchMode) {
        FragmentSupport topFragment = getTopFragment(fragmentManager);
    ...
    private void handleNewBundle(FragmentSupport toFragment, Fragment stackToFragment) {
        ...
        ((FragmentSupport) stackToFragment).onNewBundle(args);
    }
    ...
    private void debouncePop(FragmentSupport from, FragmentManager fragmentManager) {
        // Debounce
        if (from == null) {
    ...
    void popTo(String fragmentTag, boolean includeSelf, Runnable afterPopTransactionRunnable, FragmentManager fragmentManager) {
        ...
        FragmentSupport fromFragment = getTopFragment(fragmentManager);
     ...
     */
    private void handlePopAnim(Fragment targetFragment, FragmentSupport fromFragment, FragmentSupport toFragment) {
        ...
        FragmentSupport preFragment = null;
