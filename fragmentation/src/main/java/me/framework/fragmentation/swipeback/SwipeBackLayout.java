...
public class SwipeBackLayout extends FrameLayout {
    ...
    public void setFragment(final FragmentSupport fragment, View view) {
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mScrollPercent > 1) {
                if (mFragment != null) {
                    ...
                    if (mPreFragment instanceof FragmentSupport) {
                        ((FragmentSupport) mPreFragment).mLocking = true;
                    }
                    if (!mFragment.isDetached()) {
                        mFragment.popForSwipeBack();
                    }
                    if (mPreFragment instanceof FragmentSupport) {
                        ((FragmentSupport) mPreFragment).mLocking = false;
                    }
                } else {
